...
source(here::here(
  "scripts",
  "20250416-import-pafabsdaly-temp-2021.R"
))

names(absDALY2021_temp)
...
total_daly_costcal_2021 <- total_daly_costcal_2021 |> 
  mutate(totalcost_avgtemphigh=paf_daly_avgtemp975*cpd_2021)
...
saveRDS(
  ...
  here::here(
    "output",
    "temp_daly_totalburden_cvd_2021.rds"
  )
)
...
write.csv(
  ...
  here::here(
    "output",
    "temp_daly_totalburden_cvd_2021.csv"
  ),
  row.names = FALSE
)
...
total_daly_costcal_2021 |> 
  ...
  # sort the bars
  ggplot(aes(x=reorder(State,totalcost_avgtemphigh), y=totalcost_avgtemphigh)) +
