...
output_dir <- here::here("output","cvd_inci", "state_forecasts")
...
for (state in states) {
  ...
  state_data <- gbd_data_uncorrected |>
    filter(State == state) |>
    select(Year, CVD_Incidence)
  
  # Create time series
  state_ts <- ts(state_data$CVD_Incidence,
  ...
  forecasted_df <- data.frame(
    State = state,
    Year = 2020:2030,
    Forecasted_CVD_Incidences = as.numeric(forecasted_values$mean),
...
write_csv(all_forecasts, here::here("output", "cvd_inci","combined_forecasts_all_states.csv"))

# Print completion message
cat("All state forecasts have been combined and saved.\n")


all_forecasts <- read_csv(here::here("output", "cvd_inci","combined_forecasts_all_states.csv"))
