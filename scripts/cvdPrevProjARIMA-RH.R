...
output_dir <- here::here("output","cvd_prev", "state_forecasts")
...
for (state in states) {
  ...
  state_data <- gbd_data_uncorrected |>
    filter(State == state) |>
    select(Year, CVD_Prevalence)
  
  # Create time series
  state_ts <- ts(state_data$CVD_Prevalence,
  ...
  forecasted_df <- data.frame(
    State = state,
    Year = 2020:2030,
    Forecasted_CVD_Prevalences = as.numeric(forecasted_values$mean),
...
write_csv(all_forecasts, here::here("output", "cvd_prev","combined_forecasts_all_states.csv"))

# Print completion message
cat("All state forecasts have been combined and saved.\n")


all_forecasts <- read_csv(here::here("output", "cvd_prev","combined_forecasts_all_states.csv"))
